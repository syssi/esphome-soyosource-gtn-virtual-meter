...
void SoyosourceVirtualMeter::on_soyosource_modbus_data(const std::vector<uint8_t> &data) {
  ESP_LOGW(TAG, "[%s]: Unknown message received!", this->parent_id_);
}
...
void SoyosourceVirtualMeter::setup() {
  this->power_sensor_->add_on_state_callback([this](float state) {
    ...
    ESP_LOGD(TAG, "[%s]: New calculated demand: %d / last demand: %d", this->parent_id_, this->power_demand_,
...
void SoyosourceVirtualMeter::update() {
  ...
  if (this->manual_mode_switch_ != nullptr && this->manual_mode_switch_->state) {
    ...
    if (millis() - this->last_power_demand_received_ < (this->power_sensor_inactivity_timeout_s_ * 1000)) {
      ...
      ESP_LOGW(TAG, "[%s]: No power sensor update received since %d seconds. Shutting down for safety reasons.",
               this->parent_id_, this->power_sensor_inactivity_timeout_s_);
    }
  }
  ...
  ESP_LOGD(TAG, "[%s]: Setting the limiter to %d watts per inverter (%d in total)", this->parent_id_,
           power_demand_per_device, power_demand);
  this->send(power_demand_per_device);
  this->last_power_demand_ = power_demand;
  ESP_LOGVV(TAG, "[%s]: Updating last demand to: %d", this->parent_id_, this->last_power_demand_);
...
int16_t SoyosourceVirtualMeter::calculate_power_demand_negative_measurements_(int16_t consumption,
  ...
  ESP_LOGD(TAG, "[%s]: Using the new method to calculate the power demand: %d %d", this->parent_id_, consumption,
           last_power_demand);
...
int16_t SoyosourceVirtualMeter::calculate_power_demand_oem_(int16_t consumption) {
  ESP_LOGD(TAG, "[%s]: Using the dumb OEM method to calculate the power demand: %d", this->parent_id_, consumption);
