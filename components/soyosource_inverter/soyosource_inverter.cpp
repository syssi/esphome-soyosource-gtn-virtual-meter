...
void SoyosourceInverter::on_soyosource_modbus_data(const std::vector<uint8_t> &data) {
  if (data.size() != 10) {
    ESP_LOGW(TAG, "[%s]: Invalid size for soyosource status packet!", this->parent_id_);
    return;
  }
  ...
  if (battery_power > 2500 || ac_voltage > 300 || ac_frequency > 100 || temperature > 200) {
    ESP_LOGW(TAG, "[%s]: Frame dropped because of unlikely measurements!", this->parent_id_);
    return;
  }
...
void SoyosourceInverter::update() {
  if (this->no_response_count_ >= NO_RESPONSE_THRESHOLD) {
    ...
    ESP_LOGW(TAG, "[%s]: The inverter didn't respond to the last %d status requests", this->parent_id_,
